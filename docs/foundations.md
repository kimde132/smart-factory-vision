...
### 2-5. 바운딩 박스는 회전하지 않는다 — 축 정렬(axis-aligned)

2-4절의 라벨 한 줄을 다시 보면 `클래스번호 중심x 중심y 너비 높이` 다섯 개입니다. **각도를 적는 칸이 없습니다.**

그래서 박스는 **항상 화면의 가로·세로에 나란한 직사각형**입니다. 이걸 축 정렬 바운딩 박스라고 부릅니다. 기울어진 부품에도 회전한 사각형을 그리는 게 아니라, **가장 왼쪽·오른쪽·위·아래 점에 네 변이 닿는 수평 사각형**을 그립니다.
...
**"딱 맞게 그린다"가 기울어진 부품에도 성립하는 이유입니다.** 회전을 못 하니 여유를 둘 필요가 없고, 최외곽 네 점은 언제나 객관적으로 하나로 정해집니다.

> 회전하는 박스를 쓰는 방식(OBB, Oriented Bounding Box)도 있지만 이 프로젝트에서는 쓰지 않습니다. 부품 개수만 세면 되고, 각도를 알아야 할 이유가 없습니다.

### 2-6. NMS와 IoU — 겹친 박스를 정리하는 마지막 단계

**모델은 물체 하나에 박스를 하나만 내놓지 않습니다.**

YOLO는 화면을 격자로 나눠 **모든 위치에서 동시에** "여기 뭔가 있나?"를 판단합니다. 그래서 볼트 하나 주변의 여러 위치가 다 같이 "볼트 있음"이라고 답합니다.

```
실제로는 볼트 1개인데 모델이 내놓는 것:
...
이걸 정리하는 후처리가 **NMS(Non-Maximum Suppression, 비최대 억제)** 입니다. 절차는 단순합니다.

1. 확신도가 가장 높은 박스를 **확정**한다
2. 그 박스와 **많이 겹치는** 박스들을 **지운다** — 같은 물체를 가리킨다고 보고
3. 남은 박스 중 확신도가 가장 높은 것으로 1번부터 반복
4. 더 남은 게 없으면 끝

"많이 겹친다"의 기준이 **IoU(Intersection over Union, 교집합/합집합)** 입니다.
...
보통 IoU가 0.5~0.7을 넘으면 같은 물체로 봅니다.

**이 프로젝트에서 NMS가 중요한 이유.** 최종 출력이 **부품 개수**라서, NMS가 박스 하나를 잘못 지우면 개수가 틀리고 OK/NG 판정이 뒤집힙니다.

그래서 라벨을 **딱 맞게** 그려야 합니다. 여유를 두고 크게 그리면 모델도 크게 그리는 법을 배우고, 나란히 붙은 부품끼리 박스가 크게 겹쳐서 **NMS가 서로 다른 부품을 같은 물체로 오인해 지웁니다.** `labeling-guide.md` 2-1절에서 tight box를 규칙으로 정한 근거가 이것입니다.

#### 2-6-1. 추론 때 돌릴 수 있는 손잡이 셋 — `conf`, `iou`, `agnostic_nms` (2026-09-18 실측)

학습이 끝난 뒤에도 `model.predict(...)`에 넘기는 인자 셋이 **개수**를 바꿉니다. 모델을 다시 학습하지 않고 바꿀 수 있는 유일한 것들입니다.

| 인자 | 뜻 | 기본값 | 올리면 / 내리면 |
|---|---|---|---|
| `conf` | 이 확신도 **미만** 박스는 버린다 | 0.25 | 올리면 헛것이 줄고, 너무 올리면 진짜를 놓친다 |
| `iou` | NMS에서 "같은 물체"로 볼 겹침 기준. 같은 클래스 박스끼리 IoU가 이 값 **이상**이면 하나로 합친다 | 0.7 | 내리면 덜 겹쳐도 합친다 → 중복 박스가 줄지만, 실제로 겹쳐 놓인 부품 두 개를 하나로 합칠 위험 |
| `agnostic_nms` | `True`면 클래스를 무시하고 겹친 박스를 합친다 | `False` | 같은 물체에 `nut`와 `washer` 박스가 둘 다 남는 일을 막는다. 대신 살아남는 쪽이 틀린 클래스면 여전히 틀린다 |

**NMS는 기본적으로 클래스별로 따로 돕니다.** 그래서 한 와셔에 `nut 0.52`와 `washer 0.39`가 겹쳐 있어도 서로 다른 클래스라 둘 다 살아남습니다(EXP-01 `s04_008`). 같은 클래스라도 겹침이 기준 미만이면 안 합칩니다 — 긴 볼트에 "머리 부분 박스"와 "전체 박스"가 따로 나오면 IoU가 0.5 근처라 0.7 기준에 안 걸립니다(`s04_007`, `s05_035`).

**EXP-01 val·test 실측** (사진 단위 개수 정답률)

| 설정 | val(s04) 틀림 | test(s05) 틀림 |
|---|---|---|
| conf 0.25, iou 0.7 (기본) | 7장 | 2장 |
| conf 0.4, iou 0.7 | 5장 | 1장 |
| conf 0.4, iou 0.5 | 5장 | **0장** |
| conf 0.4, agnostic_nms | 5장 | 1장 (변화 없음) |

`conf`는 헛것(확신도 0.29·0.41짜리 가장자리 볼트)을 지웠고, `iou` 0.5는 볼트 중복을 합쳤습니다. `agnostic_nms`는 효과가 없었습니다 — 합쳐도 남는 박스가 틀린 클래스였기 때문입니다.
**val의 남은 5장은 이 손잡이로는 못 고칩니다.** 그림자 안 와셔를 확신도 0.85~0.93으로 너트라고 하는 것은 모델이 그렇게 배운 것이라, 데이터(s07)로 고쳐야 합니다.

운영 후보는 `conf=0.4, iou=0.5`입니다. 단 `iou`를 낮춘 상태에서 **볼트가 실제로 겹쳐 놓인 사진**을 충분히 시험한 뒤 확정합니다(s08).

### 2-7. 배경 이미지(negative sample) — 라벨이 없는 사진을 왜 넣나

**사진은 있는데 라벨 파일이 빈 상태**로 학습에 넣는 이미지를 배경 이미지 또는 negative sample이라고 합니다. 우리 프로젝트에서는 부품이 하나도 없는 빈 트레이 사진이 여기 해당합니다.

**왜 넣나.** 학습 데이터의 모든 사진에 부품이 있으면, 모델은 "사진이 들어오면 뭔가는 있다"만 경험합니다. 그 상태로 빈 트레이를 보여주면 나뭇결·얼룩·그림자를 낮은 확신도로 볼트라고 집어냅니다. **없는 것을 있다고 하는 오류를 오탐(false positive)** 이라고 합니다.

배경 이미지는 "이런 무늬는 부품이 아니다"를 가르치는 반례입니다. 개수를 세는 시스템에서는 오탐 하나가 곧 판정 오류이므로 특히 중요합니다.

**왜 많이 넣으면 안 되나.** 배경 이미지에는 박스가 없으니 **"이게 볼트다"를 가르치는 신호가 0**입니다. 절반을 넘기면 학습 신호의 절반이 "아무것도 없음"이 되고, 모델은 안전하게 아무것도 안 찾는 쪽으로 기웁니다. 그러면 실제로 있는 부품을 놓치기 시작합니다 — 이걸 **미탐(false negative)** 이라 하고, 놓치지 않고 잡아낸 비율을 **재현율(recall)** 이라고 합니다.

| 배경 이미지 비율 | 결과 |
|---|---|
| 0% | 오탐이 늘어난다 (없는 걸 있다고 함) |
| **0~10%** | **Ultralytics 권장 구간** |
| 50% 이상 | 미탐이 늘어난다 (있는 걸 놓침) |

즉 배경 이미지 비율은 **오탐과 미탐 사이의 트레이드오프**입니다. 관련 결정은 `decisions.md` D-005입니다.

### 2-8. 해상도 예산 — 부품이 사진에서 몇 픽셀로 찍히나

**아무리 고화질로 찍어도 학습할 때 이미지는 축소됩니다.** YOLO는 입력 크기를 하나로 맞추는데, 기본값이 한 변 **640픽셀**(`imgsz=640`)입니다. 4000픽셀로 찍은 사진도 이 단계에서 640으로 깎입니다.

그래서 촬영 환경을 설계할 때 이 계산을 먼저 해야 합니다.

```
1mm가 몇 픽셀?   =  640 ÷ 프레임 폭(mm)
부품이 몇 픽셀?   =  부품 크기(mm) × 위 값
```

와셔 지름 19.6mm를 예로 들면:

| 촬영 프레임 폭 | 1mm당 픽셀 | 와셔 지름 |
|---|---|---|
| 20cm | 3.2 | **63픽셀** |
| 31cm | 2.1 | 40픽셀 |
| 40cm | 1.6 | 31픽셀 |
...
**판단 기준.** 통상 **32×32 픽셀 미만**을 "작은 물체"로 분류하고, 이 구간에서는 탐지 성능이 떨어집니다. 너트의 육각 모서리처럼 **모양의 미세한 차이로 클래스를 갈라야 하면** 여유를 두고 40픽셀 이상을 목표로 잡는 게 안전합니다.

거꾸로 계산하면 **허용 가능한 최대 프레임 폭**이 나옵니다.

```
19.6mm × 640 ÷ 40픽셀 ≈ 314mm ≈ 31cm
```

프레임을 31cm보다 넓히면 와셔가 40픽셀 아래로 떨어집니다. 이게 촬영 환경 설계의 물리적 제약이 됩니다. 관련 결정은 `decisions.md` D-007입니다.

> **실전에서 꼭 지킬 것:** 사진을 눈으로 판단할 때는 **640픽셀로 줄인 뒤 100% 배율**로 보십시오.
> 4000픽셀 원본을 확대해서 보면 무엇이든 선명하게 구분됩니다. 모델이 보는 건 축소된 쪽입니다.
> (그림판 → `이미지 크기 조정` → 단위를 픽셀로 → 가로 640 → 100%로 보기)

#### 카메라 해상도는 이 공식에 나오지 않는다

위 표를 다시 보면 **카메라가 몇 만 화소인지가 어디에도 없습니다.** 처음 보면 이상한데, 이유는 단순합니다. **어차피 전부 640으로 줄이기 때문**입니다.

| | 아이폰 15 Pro | 웹캠 Kiyo X |
|---|---|---|
| 원본 해상도 | 4284 × 4284 (1835만 화소) | 1920 × 1080 (207만 화소) |
| 프레임 폭 | 22cm | 31.5cm |
| **640에서 와셔** | **57픽셀** | **40픽셀** |

화소 수는 9배 차이인데 결과는 1.4배 차이입니다. 그 1.4배도 화소 때문이 아니라 **프레임 폭(22 vs 31.5)** 때문입니다.

> 큰 종이에 그린 그림이든 작은 종이에 그린 그림이든, 똑같은 크기로 복사해 제출하면 결국 같은 크기입니다. 원본이 몇 배 컸는지는 복사 후에 남지 않습니다.

**그래서 "더 좋은 카메라를 산다"는 해결책이 아닙니다.** 원본이 `imgsz`보다 크기만 하면 되고, 그 다음부터는 프레임 폭이 전부를 결정합니다. 바꿀 수 있는 손잡이는 둘뿐입니다.

```
와셔 픽셀 = ( 19.6mm ÷ 프레임 폭 ) × imgsz
              고정        ↑손잡이 1    ↑손잡이 2
```

- **손잡이 1 — 프레임 폭.** 분모라서 좁힐수록 커진다. 카메라를 부품에 가까이 가져가는 것이다. **촬영 단계에서만 바꿀 수 있고, 찍은 뒤에는 못 바꾼다**
- **손잡이 2 — `imgsz`.** 640 → 960이면 1.5배. 대신 연산량이 `(960÷640)² = 2.25배`. **학습 설정이라 나중에 바꿔 재학습할 수 있다**

이 비대칭이 중요합니다. **프레임 폭은 되돌릴 수 없고 `imgsz`는 되돌릴 수 있습니다.** 그래서 촬영 설계를 먼저, 신중하게 합니다.

#### 화면비가 안 맞으면 픽셀을 버리게 된다 — 레터박스(letterbox)

앞의 공식에는 숨은 전제가 있습니다. **사진이 정사각형이라는 것**입니다.

YOLO는 어떤 사진이든 **정사각 캔버스**(640×640)에 넣습니다. 정사각이 아닌 사진은 **긴 변을 캔버스에 맞추고 남는 곳을 회색으로 채웁니다.** 이것을 **레터박스**라고 합니다.

```
아이폰 4284×4284 (1:1)        웹캠 1920×1080 (16:9)
┌─────────────┐               ┌─────────────┐
│             │               │░░░ 회색 140 ░│
│  640 × 640  │               ├─────────────┤
│  꽉 채움     │               │  640 × 360  │
...
│             │               │░░░ 회색 140 ░│
└─────────────┘               └─────────────┘
   와셔 57px                  캔버스의 44%가 여백
                                 와셔 40px
```

**16:9 사진은 640×640 캔버스의 56%만 씁니다.** 나머지는 아무 정보도 없는 회색입니다.

그래서 **정사각형으로 잘라내면(크롭) 그 여백이 통째로 회수됩니다.** 웹캠 1920×1080을 가운데 1080×1080으로 자르면:

```
와셔 121px × (640 ÷ 1080) = 71픽셀      ← 40px에서 71px로
```

**잘라내는 것은 좌우의 책상이지 트레이가 아니므로, 정보 손실 없이 픽셀이 회복됩니다.** 다만 검사 영역이 좁아지므로 부품이 다 들어가는지는 따로 확인해야 합니다.

> **주의:** 크롭을 학습 데이터에만 적용하고 운영(추론)에서는 원본을 넣으면, 모델이 처음 보는 화면비를 받게 됩니다. **도메인 갭(2-10)을 없애려다 새로 만드는 셈**입니다. 크롭은 촬영과 운영 양쪽에 똑같이 적용되어야 합니다.

#### 카메라 높이와 보이는 넓이는 정비례한다

프레임 폭을 바꾸는 방법은 **카메라를 올리거나 내리는 것**입니다. 시야각은 렌즈가 결정하므로 고정이고, 빛이 부채꼴로 퍼지기 때문에 **멀어질수록 밑변이 그만큼 넓어집니다.**
...
**높이를 14% 올리면 가로도 세로도 14% 넓어집니다.** 한쪽만 바뀌지 않습니다. 그래서 "가로를 좁히려고 카메라를 내리면 세로도 같이 줄어드는" 상황이 생깁니다 — 16:9 웹캠에서 이게 문제가 됩니다.

이 성질을 이용하면 목표 프레임을 역산할 수 있습니다.

| 단계 | 가로 | 세로 |
|---|---|---|
| 웹캠 현재 (16:9 전체) | 31.5cm | 17.5cm |
| 높이를 14% 올림 (`20 ÷ 17.5`) | 36cm | **20cm** |
| 가운데를 1:1로 크롭 | **20cm** | 20cm |

세로를 20cm로 맞추려고 높이를 올렸더니, 가운데를 정사각으로 자르면 가로도 자동으로 20cm가 됩니다. **같은 픽셀 수(1080)를 잘라내므로 실제 길이도 같아지기 때문**입니다.

### 2-9. 데이터를 왜 세 덩어리로 나누나 — Train / Val / Test
...
### 2-10. 도메인 갭 — 학습한 사진과 실제로 들어올 사진이 다르다
...
### 2-11. 학습에 GPU가 왜 필요한가 — CUDA와 Colab
